...
import { Bell, BookOpen, ChevronDown, ClipboardList, FileText, Home, LogOut, ScanLine, Search, Settings, Users, type LucideIcon } from "lucide-react"
...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(6,182,212,0.08),transparent_40%),linear-gradient(180deg,#f8fafc_0%,#f1f5f9_100%)] font-sans text-slate-900">
      <aside className="flex w-64 shrink-0 flex-col border-r border-slate-200/60 bg-white/60 backdrop-blur-xl transition-all">
        <div className="flex h-20 items-center border-b border-slate-200/60 px-6">
          <div className="mr-3 flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-500 text-white shadow-md shadow-cyan-500/20">
              ...
              <NavLink key={item.id} to={item.to}>
                {({ isActive }) => (
                  ...
                  >
                    ...
                    {isActive ? <motion.div layoutId="activeNavIndicator" className="ml-auto h-1.5 w-1.5 rounded-full bg-cyan-500" /> : null}
                  </div>
                )}
              </NavLink>
            )
        ...
        <div className="border-t border-slate-200/60 p-4">
          ...
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-cyan-500 to-blue-500 text-sm font-bold text-white shadow-md">
              {initials}
            </div>
      ...
      <main className="flex h-screen flex-1 flex-col overflow-hidden">
        <header className="z-10 flex h-20 shrink-0 items-center justify-between border-b border-slate-200/60 bg-white/40 px-8 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
            <button className="relative cursor-pointer rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900">
              <Bell className="h-5 w-5" />
              <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full border-2 border-white bg-rose-500" />
            </button>
            <button className="flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50">
              New Scan
